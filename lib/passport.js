...
// TODO: Maybe make mailing into it's own file?
...
module.exports = function (passport) {
  ...
  passport.deserializeUser(function (id, done) {
    Users.findById(id).then(function (user) {
      ...
      global.logger.error('Problem occured when trying to desearilize user. Message:' + err.message);
      done(null, false);
    });
  });
    ...
    process.nextTick(function () {
      ...
      Users.findOrCreate({
        ...
        if (created) {
          ...
          global.logger.info('A user with the e-mail of ' + email + ' has registered!');
          done(null, user);
        } else {
          done(null, false, req.flash('registerMessage', 'That E-mail is already taken.'));
        }
      }).catch(function (err) {
        global.logger.error('Problems occured when trying to register user. Message:' + err.message);
  ...
  passport.use('local-login', new LocalStrategy({
    ...
    Users.findOne({where: {
      ...
      if (!user) {
        ...
        if (user.password) {
          ...
          if (!Tools.validPassword(password, user.password)) {
            global.logger.warn('User id ' + user.id + ' attempted to login but entered the wrong password. Mhmm?');
      ...
      global.logger.error('Problems occured when trying to login a user. Message:' + err.message);
