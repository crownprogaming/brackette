...
module.exports = {
  ...
  isLoggedIn: function (req, res, next) {
    global.logger.info('Checking if this person is logged in ->' + req.isAuthenticated());
  ...
  redirectIfLoggedIn: function (req, res, next) {
    global.logger.info('Is user logged in?: -> ' + req.isAuthenticated());
  ...
  createTournament: function (req, res, next) {
    var user = req.user;
    ...
    Tournaments.create({
      ...
      venueAddress: newTournament.venueAddress,
      details: newTournament.details,
      rules: newTournament.rules,
      contactInfo: newTournament.contactInfo,
      createdAt: new Date(),
      updateAt: new Date()
    }).then(function (tournament) {
      global.logger.info('A new tournament (id ' + tournament.id + ') was created by user id ' + user.id + '!');
      res.send({redirect: '/'});
    }).catch(function (err) {
      global.logger.error('User id ' + user.id + 'attempted to create a tournament but an error occured. Message:' + err.message);
    });
  }
/*
